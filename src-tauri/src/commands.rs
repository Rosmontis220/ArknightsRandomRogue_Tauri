...
///
/// The only step so far is version 1 → 2: the interface language used to live under
/// `appearance`, and moving it into its own section would otherwise drop the choice and
/// reset anyone who had picked English back to Chinese. `AppearanceState::locale` is
/// still deserialised for exactly that reason, and is never written again.
///
/// Everything else added in version 2 has `#[serde(default)]`, so an older document
/// loads with the new fields at their defaults and needs no help here.
...
mod tests {
    ...
    fn a_pre_split_document_keeps_the_language_its_reader_chose() {
        // The version 1 → 2 step, and the only migration there is. Without it an English
        // reader comes back to a Chinese interface on the first launch after upgrading,
        // and the evidence of their choice — the one key version 2 stopped using — is
