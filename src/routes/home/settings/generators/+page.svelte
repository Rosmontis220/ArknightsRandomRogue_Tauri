...
<div class="flex flex-col gap-4">
	<section class="card flex flex-col gap-3 p-4">
		...
		<div class="flex flex-wrap gap-2">
			<button
				class="chip"
				data-checked={common.isJobTeamOnly}
				onclick={() => toggleFlag('isJobTeamOnly')}
			>
				{t('settings.generators.jobTeamOnly')}
			</button>
			<button
				class="chip"
				data-checked={common.isSupportUnitEnabled}
				onclick={() => toggleFlag('isSupportUnitEnabled')}
			>
				{t('settings.generators.supportUnits')}
			</button>
		</div>
	</section>
				...
				{#each generator.options as field (field.key)}
					{#if field.type === 'switch'}
						<button
							...
							data-checked={optionValue(generator.id, field) === true}
							onclick={() =>
								setOption(generator.id, field.key, optionValue(generator.id, field) !== true)}
						>
							{t(field.labelKey)}
						</button>
