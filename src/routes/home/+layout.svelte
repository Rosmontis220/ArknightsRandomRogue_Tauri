<script lang="ts">
	 ...
	 *
	 * Modelled on the reference client (`cfms_client_tauri-custom`, `home/+layout.svelte`):
	 * a 244px left rail on desktop that becomes a drawer below
	 * 820px, the selected item marked by a left accent bar, and a topbar carrying the
	 * route title.
	 *
...
<div class="shell">
	<header class="topbar">
		...
		<!-- The ID is the only input to the seed, so it stays visible on every screen
		     and clicking it is the shortcut to the field that owns it. -->
		<button
			class="identity"
			title={t('nav.identityHint')}
			onclick={() => void goto('/home/settings/identity')}
		>
			<span class="identity-label dim">{t('nav.identity')}</span>
