 ...
 * pure function of `md5(name + "_" + today)`. This module keeps that promise
...
/**
 * Derives the run's 32-bit seed: `parseInt(md5(`${name}_${date}`).slice(0, 8), 16)`.
 *
...
export function deriveHash(name: string, date: string): number {
	return parseInt(md5(`${name}_${date}`).slice(0, 8), 16);
}
...
export interface Seed {
	...
	/**
	 * A child seed: `md5(`${value}_${salt}`)` truncated to 32 bits.
	 *
...
export function makeSeed(value: number): Seed {
	return {
		...
		derive: (salt) => makeSeed(parseInt(md5(`${value}_${salt}`).slice(0, 8), 16))
	};
}
