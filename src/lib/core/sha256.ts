 ...
 * under us via a lockfile bump. It also replaces the MD5 the app started with, which
 * is no longer used anywhere: the predecessor tool's digest was never a contract this
 * app may copy, and a broken hash is the wrong thing to keep for the sake of
 ...
 *
 * Unlike MD5, SHA-256 is big-endian throughout: both the length field and the digest
 * words are written most-significant byte first (FIPS 180-4 §5.1.1). Getting that
 * backwards produces a plausible-looking but wrong digest, so it is called out here
 * and asserted by the vectors.
 *
 ...
 * the same digest everywhere: `sha256('逗比寒')` is one fixed value on any platform.
 */
export function sha256(input: string): string {
	...
	// modulo 64; the final 8 bytes hold the bit length big-endian, unlike MD5.
...
/**
 * The digest is big-endian, so each state word is emitted highest byte first — the
 * opposite of MD5. Every word is already kept unsigned (`>>> 0`), so no masking is
 * needed here.
 */
