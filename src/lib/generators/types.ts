...
export type ResultBlock =
	 ...
	 *
	 * `tone: 'ban'` marks the block whose operators are *banned* rather than merely
	 * listed. The result renderer needs to tell the two apart — with portraits on,
	 * a banned operator's portrait gets a grey translucent mask, while 老缠杯's pool
	 * (also a list of operator names) must not — and inferring it from the text shape
	 * would get that wrong, since both are just names.
	 */
	| { type: 'copyable'; label: TextSpec; text: TextSpec; tone?: 'ban' }
	/**
