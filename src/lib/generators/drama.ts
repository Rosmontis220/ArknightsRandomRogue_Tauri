...
export function dramaBlocks(summary: TextSpec, outcome: DramaOutcome): ResultBlock[] {
	...
	if (outcome.bans.length > 0) {
		blocks.push({
			type: 'copyable',
			// The one block that carries a tone: these operators are *banned*, and the
			// result renderer greys their portraits out when portrait mode is on. Every
			// other name list — 老缠杯's pool, the per-class picks — is a plain list.
			tone: 'ban',
			label: {
				kind: 'key',
