 ...
 * function. With no rules this is the plain drama draw, which `opening.test.ts` pins
 * across a fixed 3072-case input matrix.
 */

import { getRogueName, getRogueTeam, JOB_TEAM_LIST } from '../core/rogues';
import { t } from '../i18n';
import { namesAsText, picksAsText } from './format';
...
export function runDrama(
	...
	const userExcluded = new Set(ctx.box.excluded);
	const excluded = new Set([...userExcluded, ...preBanned]);

	const box = buildBox(operators, excluded);
	// Listed separately from `excluded`: a pre-banned operator *is* in the user's
	// box, and saying otherwise would be a lie about their own settings.
	const notInBox = operators.star6.filter((name) => userExcluded.has(name));
	...
	const teamName = ctx.common.isJobTeamOnly
		? seed.pick(JOB_TEAM_LIST)
		: getRogueTeam(rogueName, seed.value);
	...
	const openingOperator = seed.pick(candidates);
...
export function dramaBlocks(summary: TextSpec, outcome: DramaOutcome): ResultBlock[] {
	...
	if (Object.keys(outcome.picks).length > 0) {
		blocks.push({
			type: 'copyable',
			label: { kind: 'key', key: 'result.picksHeading' },
			text: { kind: 'picks', picks: outcome.picks }
		});

		// One checklist per job: `id` doubles as the tick-store key and the group
		// heading (see `ResultBlock`). The heading is a `name` spec so the job renders
		// as a class name in English; the `id` stays Chinese because it keys the ticks.
		for (const [job, names] of Object.entries(outcome.picks)) {
			blocks.push({
				type: 'checklist',
				id: `picks:${job}`,
				items: names,
				label: { kind: 'name', name: job }
			});
		}
	}
...
export function dramaFields(outcome: DramaOutcome): Record<string, string> {
	return {
		...
		picks: picksAsText(outcome.picks)
	};
}
