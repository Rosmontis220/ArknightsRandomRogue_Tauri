<script lang="ts">
	 ...
	 *
	 * Also read from `tone`, for the same reason: the pool is a list of names that must stay
	 * inert — an operator you have not picked cannot be "used".
	 */
...
{#snippet nameRow(blockIndex: number, names: readonly string[], masked: boolean, markable: boolean)}
		...
		{:else if block.type === 'copyable'}
			...
			<div>
				...
				{#if operatorNames !== null && (avatarMode || markable)}
					<div class="mt-1">
						{@render nameRow(index, operatorNames, isBanned(block), markable)}
					</div>
				{:else}
