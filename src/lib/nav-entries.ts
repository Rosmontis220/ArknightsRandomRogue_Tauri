 ...
 *
 * 设置 lands on a list of sections. 生成 lands on three full-width buttons — one per
 * mode — and a mode holding several editions (`?f=xianshu`) opens a second list.
...
/** The settings root's list: one ungrouped run of six sections. */
 ...
 * it. Eleven editions of one cup under a single heading buried the other two modes, and
 * the landing screen's job is to ask which mode, not which numbered event.
 *
