...
const ZH_CN: Record<string, string> = {
	...
	'generator.laochan.name': '老缠杯 #2',
	'generator.laochan.description': '整局只能用抽出的 10 位六星',
	...
	/* Result text: composed from keys rather than inside the generators, so a result can
	   ...
	   `.one` / `.other` variant. */
	'result.summary.opening': '我掐指一算，今天 {name} 适合在 {rogue} 用 {team} {operator} 开局',
	'result.summary.cup': '{cup}：今天 {name} 固定 {rogue}，用 {team} {operator} 开局',
	'result.summary.laochan.one': '我掐指一算，今天 {name} 这一局只能使用以下 {count} 位六星干员',
	'result.summary.laochan.other': '我掐指一算，今天 {name} 这一局只能使用以下 {count} 位六星干员',
	...
	'result.laochanEasterEgg': '检测到彩蛋 ID，抽取池已收窄为 {first} 与 {second}。',
	...
	/* Settings --------------------------------------------------------------- */
	'settings.identity.title': 'ID',
	'settings.identity.summary': 'ID 是种子的唯一来源，改它就换一套结果',
	...
	'settings.box.help': '勾选表示「在 box 中」。存的是排除项，所以以后新增的干员默认在 box 里。',
	...
	'settings.appearance.summary': '主题配色、语言、动效与启动画面',
	...
	'about.featureCups': '收录仙术杯 #1 至 #9 各届数，以及老缠杯 #2',
...
const EN: Record<string, string> = {
	...
	'generator.laochan.name': 'Laochan Cup #2',
	'generator.laochan.description': 'The whole run uses only the ten six-stars drawn',
	...
	'result.summary.opening':
		'I cast the runes: today {name} should open in {rogue} with {team} and {operator}',
	'result.summary.cup':
		'{cup}: today {name} is locked to {rogue}, opening with {team} and {operator}',
	'result.summary.laochan.one':
		'I cast the runes: {name} may use only the one six-star below this run',
	'result.summary.laochan.other':
		'I cast the runes: {name} may use only the {count} six-stars below this run',
	...
	'result.laochanEasterEgg': 'Easter-egg ID detected: the pool is narrowed to {first} and {second}.',
	...
	/* Settings --------------------------------------------------------------- */
	'settings.identity.title': 'ID',
	'settings.identity.summary': 'The ID is the seed’s only source — change it and every result changes',
	...
	'settings.box.help':
		'Ticked means “in box”. What is stored is the exclusions, so an operator added later is in the box by default.',
	...
	'settings.appearance.summary': 'Theme, language, motion and splash screen',
	...
	/* App shell, about and data ---------------------------------------------- */
	'app.title': 'Arknights Random Rogue Generator',
	...
	'about.featureCups': 'Covers Theurgy Cup #1 through #9, plus Laochan Cup #2',
